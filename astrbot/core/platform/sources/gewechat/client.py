...
class SimpleGewechatClient:
    ...
    async def _convert(self, data: dict) -> AstrBotMessage:
        ...
        data_parser = GeweDataPaser(content, abm.group_id == "")
        # 不同消息类型
        match d["MsgType"]:
            case 1:
                # 文本消息
            ...
            case 47:  # emoji
                ...
                logger.info(f"消息类型(47)：emoji, {content}")
            case 48:  # 地理位置
    ...
    async def post_emoji(self, to_wxid, emoji_md5, emoji_size, cdnurl=""):
        ...
        try:
            if emoji_md5 != "" and emoji_size != "":
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                            f"{self.base_url}/message/postEmoji", headers=self.headers, json=payload
                    ) as resp:
                        json_blob = await resp.json()
                        logger.info(f"发送emoji消息结果: {json_blob.get('msg', '操作失败')}")
            else:
